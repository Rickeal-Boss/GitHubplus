...
namespace FastGithub
{
    ...
    sealed class AppHostedService : BackgroundService
    {
        ...
        private async Task WaitForParentProcessExitAsync(CancellationToken cancellationToken)
        {
            ...
            // [PATCH] 上游把 GetProcessById 的一切异常都记 Error（"获取进程{N}异常"）。
            // 其中 ArgumentException（进程不存在）是预期分支而非故障：UI 快速重启场景下，
            // 引擎启动 1 秒后发现锚点（ping）已随上一个 UI 实例退出而死亡，语义上等价于
            // "父进程已退出"，应走优雅自停路径。真机日志（pre17，10 秒内 4 连环重启）该 ERR
            // 连续出现 4 次，污染错误监控并误导排障方向。此处单独降为 Information；
            // 其余异常（真正的获取失败）仍记 Error。
            // 顺带用 using 释放 GetProcessById 返回的 Process 句柄（上游泄漏）。
            try
            {
            ...
            {
                this.logger.LogInformation($"父进程（PID={parentId}）已不存在，按其已退出处理");
            }
            catch (Exception ex)
            {
