        ...
        public async IAsyncEnumerable<IPAddress> ResolveAsync(DnsEndPoint endPoint, [EnumeratorCancellation] CancellationToken cancellationToken)
        {
            ...
            else
            {
                ...
                await foreach (var address in this.dnsClient.ResolveAsync(endPoint, fastSort: true, cancellationToken))
                {
                    ...
                    // 提前终止：IPv4 满员即停，或候选总数达到 MAX_IP_COUNT × 2 封顶即停。
                    // 只按 IPv4 计数会在「首个 DNS 服务器返回的以 AAAA 为主」时永不满足，从而把
                    // 后续每个 DNS 服务器都轮询一遍（单个约 4 秒，冷解析最坏从约 4 秒升到约 20 秒）；
                    // 总数封顶用来兜住这种情形，不改动最终名单的构成规则。
                    ...
                    var ipv4Count = candidates.Count(item => item.AddressFamily == AddressFamily.InterNetwork);
                    if (ipv4Count >= MAX_IP_COUNT || candidates.Count >= MAX_IP_COUNT * 2)
                    {
                        break;
                    }
                }
