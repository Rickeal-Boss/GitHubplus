    ...
    sealed class CaCertInstallerOfWindows : ICaCertInstaller
    {
        ...
        public void Install(string caCertFilePath)
        {
            try
            {
                ...
                // 原来按主题名查找，而该查找方式是「子串匹配」，会命中
                // LocalMachine\Root 下任何主题包含该字符串的证书并删除，误删第三方根证书
                // 会导致相关站点全部证书校验失败。改为遍历后精确比对：
                // 完整主题 DN 相同 + 自签名(颁发者等于主题) + 指纹不同，才删除旧的。
                foreach (var item in store.Certificates)
                {
                    if (item.SubjectName.Name == caCert.SubjectName.Name &&
                        item.IssuerName.Name == item.SubjectName.Name &&
                        item.Thumbprint != caCert.Thumbprint)
                    {
                        store.Remove(item);
                    }
                }
